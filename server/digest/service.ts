...
function renderDigest(sqlite: Database.Database, settings: DigestSettings, now: Date): Omit<MailMessage, "from" | "to"> {
  ...
  const lines = items.length ? items.map((item) => `- ${item.card}: ${item.plan} (${item.state.replace("_", " ")}${item.dueOn ? `, ${item.dueOn}` : ""})`) : ["Nothing is due soon. Your yard is caught up."];
  const rows = items.length ? items.map((item) => `<tr><td style="padding:8px;border-bottom:1px solid #ddd">${escapeHtml(item.card)}</td><td style="padding:8px;border-bottom:1px solid #ddd">${escapeHtml(item.plan)}</td><td style="padding:8px;border-bottom:1px solid #ddd">${escapeHtml(item.state.replace("_", " "))}${item.dueOn ? ` · ${item.dueOn}` : ""}</td></tr>`).join("") : `<tr><td style="padding:12px" colspan="3">Nothing is due soon. Your yard is caught up.</td></tr>`;
  return {
    subject: `${settings.displayName}: ${items.length ? `${items.length} items need attention` : "all caught up"}`,
    text: [`Yard Tracker — ${settings.displayName}`, "", ...lines, "", "Open Yard Tracker on your household network for details."].join("\n"),
    html: `<div style="font-family:system-ui,sans-serif;color:#1d332b;max-width:640px"><h1 style="font-family:Georgia,serif">${escapeHtml(settings.displayName)}</h1><p>${items.length ? `${items.length} maintenance items need attention.` : "Everything is on schedule."}</p><table style="border-collapse:collapse;width:100%"><tbody>${rows}</tbody></table><p style="color:#68776f;font-size:13px">Open Yard Tracker on your household network for details.</p></div>`,
  };
}
